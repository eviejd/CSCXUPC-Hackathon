...
function wireEvents() {
  var el;
  el = $("addUserBtn");
  if (el) el.addEventListener("click", addUser);
  el = $("userNameInput");
  if (el) el.addEventListener("keypress", function (e) {
    if (e.key === "Enter") addUser();
  });
  el = $("addTaskBtn");
  if (el) el.addEventListener("click", addTask);
  el = $("taskInput");
  if (el) el.addEventListener("keypress", function (e) {
    if (e.key === "Enter") addTask();
  });
  el = $("btnStartRound");
  if (el) el.addEventListener("click", startRoundOnServer);
  el = $("submitBidBtn");
  if (el) el.addEventListener("click", submitBidToServer);
  el = $("bidAmount");
  if (el) el.addEventListener("keypress", function (e) {
    if (e.key === "Enter") submitBidToServer();
  });
  el = $("nextRoundBtn");
  if (el) el.addEventListener("click", startRoundOnServer);
  ...
  if (usersList) {
    if (users.length) {
      ...
      for (var i = 0; i < users.length; i++) {
        var u = users[i];
        html += '<div class="list-item"><div class="user-name">' + u.name + "</div><div class=\"user-points\">" + (u.points != null ? u.points : 100) + " pts</div></div>";
      }
